...
- Git フックは prek（pre-commit 互換で Python 不要）。`.pre-commit-config.yaml` に terraform fmt / validate / tflint、gitleaks、ryl（yamllint 互換の Rust 実装）、rumdl（Markdown）、shellcheck、actionlint を並べる。ryl と rumdl は mise で入れた実行ファイルを `language: system` で呼ぶ
